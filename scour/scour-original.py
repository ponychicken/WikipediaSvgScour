...
def removeFlow(element, options, doc) :
	...
	#print text
	#print '\n'
	# texttag = doc.createElement('text')
	# doc.insertBefore(texttag, element)
	# texttag.setAttribute(id, id)
	# texttag.NodeValue = 'test'
	# 
...
#	out_string = doc.documentElement.toprettyxml(' ')
	...
	total_output = total_output.replace('flowRoot','text')
	#total_output = total_output.replace('<text','<text font-family="Kochi Gothic"')
	total_output = re.sub('onmouseover=".*?"','', total_output)
	total_output = re.sub('onmouseout=".*?"','', total_output)	
	return total_output
